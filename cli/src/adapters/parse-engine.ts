...
export class ParseEngine {
  ...
  /** Check if parse engine detected resume TUI on screen */
  isResumeTuiActive(): boolean {
    return !!this.ctx.resumeTuiActive
  }

  /** Clear resume TUI signal (after ws-server has consumed it) */
  clearResumeTuiSignal(): void {
    this.ctx.resumeTuiActive = false
  }
