...
export function buildAutomationSocialInstructions(mode: SocialAutomationMode): string {
  ...
  return [
    ...
    "If the approved copy exceeds the platform character limit, submit it as-is — the daemon will automatically split it into a reply thread. Do NOT skip, truncate, or rewrite long posts yourself.",
    "If the approved copy would need OTHER edits (not length) before posting, emit a skip marker instead of rewriting it yourself.",
    "",
    "If a post should be published now, end your final output with exactly one line:",
