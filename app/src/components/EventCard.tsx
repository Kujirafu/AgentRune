...
const STATUS_COLORS: Record<string, string> = {
  waiting: "rgba(250,204,21,0.6)",
  in_progress: "rgba(96,165,250,0.6)",
  completed: "rgba(74,222,128,0.6)",
  failed: "rgba(248,113,113,0.6)",
}

const TYPE_LABEL_KEYS: Record<string, string> = {
  file_edit: "event.fileEdit", file_create: "event.fileCreate", file_delete: "event.fileDelete",
  command_run: "event.commandRun", test_result: "event.testResult", install_package: "event.installPackage",
  decision_request: "event.decisionRequest", error: "event.error", info: "event.info", session_summary: "event.sessionSummary",
}

function stripAnsi(s: string): string {
  return s.replace(/\x1b\[[0-9;?]*[a-zA-Z]/g, "").replace(/\x1b\][^\x07]*(?:\x07|\x1b\\)/g, "").replace(/\x1b[78]/g, "").replace(/\x1b\([A-Z]/g, "").replace(/[\x00-\x08\x0b\x0c\x0e-\x1f]/g, "")
}

function stripStatusBar(s: string): string {
  return s.replace(/[*\u2217\u2234]\s*[A-Z][a-zA-Z]*ing\b[^❯]*/g, "").replace(/\(running\s+stop\s+hooks[^)]*\)/gi, "").replace(/\(\d+\.?\d*s\s*·[^)]*\)/g, "").replace(/\(\d+\.?\d*s\s*[·.]\s*[↑↓]?\s*\d[\d,]*\s*tokens?\)/gi, "").replace(/[↑↓]\s*\d[\d,]*\s*tokens?[^❯]*/gi, "").replace(/thought\s+for\s+\d+s/gi, "").replace(/\d+\s*tokens?\s*used/gi, "").replace(/[❯>$%]\s*$/, "").replace(/\s{2,}/g, " ").trim()
}
...
export function EventCard({ event, onDecision, onQuote, onSaveObsidian, onViewDiff }: EventCardProps) {
  ...
  const onPointerDown = useCallback((e: React.TouchEvent | React.MouseEvent) => {
    ...
    longPressTimer.current = setTimeout(() => { longPressFired.current = true; handleContextMenu(pos.x, pos.y) }, 500)
  }, [handleContextMenu])
  ...
  if (isUserMsg) {
    return (
      <>
        <div onClick={() => { if (hasDetail) setExpanded(!expanded) }}
          onTouchStart={onPointerDown} onTouchEnd={onPointerUp} onTouchMove={onPointerMove}
          onContextMenu={(e) => { e.preventDefault(); handleContextMenu(e.clientX, e.clientY) }}
          style={{
            padding: "6px 12px", borderRadius: 12, background: "rgba(59,130,246,0.08)",
            borderRight: "3px solid rgba(59,130,246,0.5)", animation: "fadeSlideUp 0.3s ease-out",
            textAlign: "right", display: "flex", flexDirection: "column", gap: 4,
            userSelect: "none", WebkitUserSelect: "none", cursor: hasDetail ? "pointer" : "default",
          }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "flex-end", gap: 8 }}>
            <span style={{ fontSize: 10, color: "var(--text-secondary)", opacity: 0.5, flexShrink: 0 }}>
              {new Date(event.timestamp).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}
            </span>
            <span style={{ fontSize: 13, fontWeight: 600, color: "var(--accent-primary)", fontFamily: "'JetBrains Mono', monospace" }}>
              {event.title}{hasDetail && !expanded && <span style={{ opacity: 0.5, marginLeft: 4 }}>{"\u25BC"}</span>}
            </span>
          </div>
          {expanded && event.detail && (
            <div style={{ fontSize: 12, color: "var(--accent-primary)", fontFamily: "'JetBrains Mono', monospace", textAlign: "left", whiteSpace: "pre-wrap", wordBreak: "break-word", opacity: 0.85, padding: "6px 0 2px", borderTop: "1px solid rgba(59,130,246,0.15)", marginTop: 2 }}>
              {event.detail}
            </div>
          )}
        </div>
  ...
  return (
    <>
      <div onClick={handleClick}
        ...
        onContextMenu={(e) => { e.preventDefault(); handleContextMenu(e.clientX, e.clientY) }}
        style={{
          padding: event.decision ? 16 : "8px 12px", borderRadius: event.decision ? 20 : 14,
          ...
          boxShadow: "var(--glass-shadow)", cursor: event.decision ? "default" : "pointer",
          transition: "all 0.2s ease", animation: "fadeSlideUp 0.3s ease-out",
          userSelect: "none", WebkitUserSelect: "none", overflow: "hidden", maxWidth: "100%", flexShrink: 0,
        }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: event.decision ? 8 : event.detail ? 4 : 0 }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{
              fontSize: 13, fontWeight: 500, color: "var(--text-primary)", whiteSpace: "normal", wordBreak: "break-word",
              display: "-webkit-box", WebkitLineClamp: 3, WebkitBoxOrient: "vertical" as never, overflow: "hidden",
            }}>
              {event.title}
            </div>
          </div>
          <span style={{ fontSize: 10, color: "var(--text-secondary)", fontWeight: 500, flexShrink: 0, opacity: 0.7 }}>
        ...
        {cleanDetail && (
          <div>
            <div ref={detailRef} style={{ fontSize: 13, color: "var(--text-primary)", maxHeight: detailExpanded ? "none" : 120, overflow: "hidden" }}>
              {cleanDetail.startsWith("__IMG__") ? (
                <img src={cleanDetail.slice(7)} alt="uploaded" style={{ maxWidth: "100%", maxHeight: 300, borderRadius: 10, objectFit: "contain" }} />
              ) : (
                <>
                  <style>{`
                  ...
                  `}</style>
                  <div className="ec-md"><ReactMarkdown>{cleanDetail}</ReactMarkdown></div>
                </>
              )}
            </div>
            {!detailExpanded && (detailOverflow || cleanDetail.length > 300) && (
              <div onClick={(e) => { e.stopPropagation(); setDetailExpanded(true) }} style={{ textAlign: "center", cursor: "pointer", paddingTop: 4, marginTop: 2, borderTop: "1px solid var(--glass-border, rgba(255,255,255,0.1))" }}>
                <span style={{ fontSize: 10, color: "var(--accent-primary)", fontWeight: 600, opacity: 0.8 }}>{"\u25BC"} {t("event.showMore") || "Show more"}</span>
              </div>
            )}
            {detailExpanded && cleanDetail.length > 300 && (
              <div onClick={(e) => { e.stopPropagation(); setDetailExpanded(false) }} style={{ textAlign: "center", cursor: "pointer", paddingTop: 4 }}>
                <span style={{ fontSize: 10, color: "var(--accent-primary)", fontWeight: 600, opacity: 0.8 }}>{"\u25B2"} {t("event.showLess") || "Show less"}</span>
              </div>
            )}
          </div>
        )}
        ...
        {(event.type === "file_edit" || event.type === "file_create") && event.diff && onViewDiff && (
          <div style={{ marginTop: 6 }}>
            <button onClick={(e) => { e.stopPropagation(); onViewDiff(event) }} style={{
              padding: "3px 10px", borderRadius: 6, border: "1px solid rgba(96,165,250,0.25)",
              background: "rgba(96,165,250,0.07)", color: "rgba(96,165,250,0.8)", fontSize: 11, fontWeight: 600, cursor: "pointer",
            }}>
              View diff
            </button>
          </div>
        ...
        {event.decision && (
          <div style={{ display: "flex", flexDirection: "column", gap: 6, marginTop: 12 }}>
            {event.decision.options.map((opt) => (
              <button key={opt.label} onClick={() => onDecision?.(opt.input)} style={{
                width: "100%", padding: "10px 14px", borderRadius: 12,
                border: opt.style === "danger" ? "1px solid rgba(248,113,113,0.3)" : "1px solid rgba(59,130,246,0.3)",
                background: opt.style === "danger" ? "rgba(248,113,113,0.1)" : "rgba(59,130,246,0.1)",
                color: opt.style === "danger" ? "#f87171" : "var(--accent-primary)",
                fontSize: 13, fontWeight: 600, cursor: "pointer", transition: "all 0.2s",
                textAlign: "left", wordBreak: "break-word" as const, whiteSpace: "pre-wrap", overflow: "hidden",
              }}>
                {opt.label}
              </button>
            ))}
        ...
        {expanded && cleanRaw && (
          <div style={{
            marginTop: 12, padding: 12, borderRadius: 12, background: "var(--card-bg)",
            border: "1px solid var(--glass-border)", fontFamily: "'JetBrains Mono', monospace",
...
interface MenuAction { key: string; label: string; action: () => void }

function ContextMenuOverlay({ actions, pos, onClose }: { actions: MenuAction[]; pos: { x: number; y: number }; onClose: () => void }) {
  ...
  return (
    <div onClick={onClose} onTouchEnd={handleBackdropTouch} style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, zIndex: 9999 }}>
      <div onClick={(e) => e.stopPropagation()} onTouchEnd={(e) => e.stopPropagation()} style={{
        position: "absolute", top: y, left: x, width: menuW,
        ...
        padding: 10, display: "flex", flexDirection: "column" as const, gap: 2,
        transition: `all 0.3s ${SPRING}`, animation: `ctxMenuIn 0.25s ${SPRING}`,
        transformOrigin: `${pos.x - x}px ${pos.y - y}px`,
        userSelect: "none", WebkitUserSelect: "none",
      }}>
        {actions.map((a, i) => (
          <button key={a.key} onClick={() => { a.action(); onClose() }} onTouchEnd={(e) => { e.stopPropagation(); a.action(); onClose() }}
            style={{
              width: "100%", padding: "12px 14px", borderRadius: 14, border: "none",
              background: "transparent", color: "var(--text-primary)", fontSize: 14, fontWeight: 600,
              textAlign: "left", cursor: "pointer", transition: `all 0.3s ${SPRING}`,
              display: "flex", alignItems: "center", gap: 12,
              ...(i < actions.length - 1 ? { borderBottom: "1px solid var(--glass-border)", borderBottomLeftRadius: 0, borderBottomRightRadius: 0 } : {}),
            }}
            onPointerDown={(e) => { (e.currentTarget as HTMLElement).style.background = "var(--icon-bg)" }}
          ...
          >
            <div style={{
              width: 32, height: 32, borderRadius: 10, background: "var(--icon-bg)",
              border: "1px solid var(--glass-border)", display: "flex", alignItems: "center", justifyContent: "center",
              flexShrink: 0, color: "var(--accent-primary)", boxShadow: "0 2px 8px rgba(0,0,0,0.05)",
            }} dangerouslySetInnerHTML={{ __html: MENU_ICONS[a.key] || MENU_ICONS.copy }} />
            <span style={{ letterSpacing: -0.3 }}>{a.label}</span>
          </button>
        ))}
      </div>
      <style>{`@keyframes ctxMenuIn { from { opacity: 0; transform: scale(0.9) } to { opacity: 1; transform: scale(1) } }`}</style>
    </div>
  )
}
