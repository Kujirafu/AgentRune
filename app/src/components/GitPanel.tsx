...
export function GitPanel({ open, projectId, onClose }: GitPanelProps) {
  ...
  useEffect(() => {
    if (open) { fetchStatus(); setSelectedFile(null); setCommitResult(null) }
  }, [open, fetchStatus])
  ...
  const onTouchMove = useCallback((e: React.TouchEvent) => {
    ...
    if (d.dir === "h") {
      ...
      if (slideRef.current) {
        slideRef.current.style.transition = "none"
        slideRef.current.style.transform = `translateX(calc(${-diffPage * 100}% + ${offset}px))`
      }
    }
  }, [diffPage])
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 16px 8px" }}>
            <button onClick={() => { setSelectedFile(null); history.back() }}
