...
export function TaskBoard({ open, projectId, onClose, onStartTask, send }: TaskBoardProps) {
  ...
  const handleBreakdown = useCallback(() => {
    ...
    // Send the prompt to the active agent session
    ...
    // We'll listen for the response via a timeout-based approach
    // The user will see the agent working and can manually copy tasks
    // For now, create a placeholder store so user sees the UI changed
    setTimeout(() => {
      setProcessing(false)
      // Create empty store with requirement saved
  ...
  const handleImportTasks = useCallback(() => {
    try {
      // Try to extract JSON array from text (might be wrapped in markdown code block)
        ...
        {hasTasks && !showAddTasks && store.tasks.map((task) => {
          ...
          return (
            <div key={task.id} style={{
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}
                ...
                <button
                  ...
                  style={{
                    ...
                    background: sc.bg, border: `1px solid ${sc.border}`,
                    color: sc.text, fontSize: 10, fontWeight: 700,
              ...
              {/* Expanded description + start button */}
              {isExpanded && (
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button
              ...
              style={{
                ...
                fontSize: 13, fontWeight: 600, cursor: "pointer", opacity: 0.6,
              }}
            >
              + Add Requirement
            </button>
