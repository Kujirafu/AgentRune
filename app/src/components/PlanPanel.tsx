...
const isFinished = (status: string) => isDone(status) || status === "skipped"
...
export const PlanPanel = React.memo(function PlanPanel({ projectId, send }: PlanPanelProps) {
  ...
  // ── Tasks (aggregated from all PRDs + standalone) ──
  ...
  }, [projectId, standaloneTasks, fetchAllTasks])
  ...
  // Auto-refresh tasks when InputBar adds a task (fires custom event)
  useEffect(() => {
  ...
  const updateTaskStatus = useCallback((prdId: string, taskId: number, status: string) => {
    fetch(`${getApiBase()}/api/prd/${encodeURIComponent(projectId)}/${encodeURIComponent(prdId)}/tasks/${taskId}`, {
      ...
      if (data?.prdAutoCompleted) {
        ...
        // Update tasks tab local state
        setPrdTaskGroups(prev => prev.map(g =>
          g.prdId === prdId ? { ...g, tasks: g.tasks.map(t => t.id === taskId ? { ...t, status: data.task.status } : t) } : g
        ))
        fetchPrdList() // refresh PRD list counts
      }
    }).catch(() => {})
  }, [projectId, selectedPrd, fetchPrdDetail, fetchPrdList, fetchAllTasks])
          ...
          <div style={sectionStyle}>
            ...
            {prd.decisions.map((d: any, i: number) => (
              <div key={i} style={{ padding: "8px 10px", borderRadius: 8, background: cardBg, border: `1px solid ${isDark ? "rgba(255,255,255,0.05)" : "rgba(0,0,0,0.04)"}`, marginBottom: 6 }}>
                {typeof d === "string" ? (
                  <div style={{ fontSize: 12, color: "var(--text-primary)", lineHeight: 1.5 }}>{d}</div>
                ) : (
                  <>
          ...
          <div style={sectionStyle}>
            ...
            {prd.approaches.map((a: any, i: number) => {
              if (typeof a === "string") return (
                <div key={i} style={{ padding: "8px 10px", borderRadius: 8, background: cardBg, border: `1px solid ${isDark ? "rgba(255,255,255,0.05)" : "rgba(0,0,0,0.04)"}`, marginBottom: 6, fontSize: 12, color: "var(--text-primary)", lineHeight: 1.5 }}>{a}</div>
              )
              return (
              <div key={i} style={{
          ...
          {tasks.map((task, idx) => {
            ...
            return (
              <div key={task.id} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 0", borderBottom: idx < tasks.length - 1 ? `1px solid ${isDark ? "rgba(255,255,255,0.04)" : "rgba(0,0,0,0.03)"}` : "none" }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 12, fontWeight: 600, color: "var(--text-primary)", textDecoration: isFinished(task.status) ? "line-through" : "none", opacity: isFinished(task.status) ? 0.5 : 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                    {task.title}
                  </div>
    ...
    return (
      <>
        ...
        {prdList.map((prd) => {
          ...
          const prdDone = isDone(prd.status)
          return (
            <div key={prd.id} onClick={() => openPrd(prd.id)}
              style={{
                padding: "12px 14px", borderRadius: 12, cursor: "pointer",
                background: prdDone ? (isDark ? "rgba(34,197,94,0.04)" : "rgba(34,197,94,0.03)") : cardBg,
                border: `1px solid ${prdDone ? "rgba(34,197,94,0.15)" : isDark ? "rgba(255,255,255,0.06)" : "rgba(0,0,0,0.05)"}`,
                backdropFilter: "blur(16px)", WebkitBackdropFilter: "blur(16px)",
                opacity: prdDone ? 0.7 : 1,
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 13, fontWeight: 600, color: "var(--text-primary)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", textDecoration: prdDone ? "line-through" : "none" }}>
                    {prd.title}
                  </div>
                </div>
                {prdDone && (
                ...
                <div style={{ marginTop: 8 }}>
                  ...
                  <div style={{ height: 3, borderRadius: 2, background: isDark ? "rgba(255,255,255,0.06)" : "rgba(0,0,0,0.06)" }}>
                    <div style={{
                      height: "100%", borderRadius: 2,
                      width: `${progress}%`,
                      background: prdDone ? "#22c55e" : "linear-gradient(90deg, #37ACC0, #347792)",
                      transition: `width 0.5s ${SPRING}`,
                    }} />
                  </div>
                </div>
    ...
    <div style={{ height: "100%", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ display: "flex", gap: 4, padding: "4px 16px 8px", flexShrink: 0 }}>
        <button onClick={() => { setActiveTab("prd"); setSelectedPrdId(null); setSelectedPrd(null) }} style={tabStyle(activeTab === "prd")}>
          {t("prd.tabTitle") || "PRD"} {prdList.filter(p => p.status === "active").length > 0 ? `(${prdList.filter(p => p.status === "active").length})` : ""}
        </button>
      ...
      <div style={{ flex: 1, overflow: "auto", padding: "0 16px 16px", display: "flex", flexDirection: "column", gap: 8 }}>
        ...
        {activeTab === "tasks" && (
          <div>
            {tasksLoading && <div style={{ fontSize: 11, color: "var(--text-secondary)", textAlign: "center", padding: 20 }}>Loading...</div>}
            {!tasksLoading && prdTaskGroups.length === 0 && standaloneTasks.length === 0 && (
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 12, padding: "24px 16px" }}>
                <div style={{ width: 48, height: 48, borderRadius: 14, background: isDark ? "rgba(55,172,192,0.08)" : "rgba(55,172,192,0.06)", border: `1px solid ${isDark ? "rgba(55,172,192,0.15)" : "rgba(55,172,192,0.1)"}`, display: "flex", alignItems: "center", justifyContent: "center" }}>
                  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#37ACC0" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                    <path d="M9 11l3 3L22 4"/><path d="M21 12v7a2 2 0 01-2 2H5a2 2 0 01-2-2V5a2 2 0 012-2h11"/>
                  </svg>
                </div>
                <div style={{ textAlign: "center" }}>
                  <div style={{ fontSize: 14, fontWeight: 700, color: "var(--text-primary)", marginBottom: 4 }}>
                    {t("tasks.emptyTitle") || "No tasks yet"}
                  </div>
                  <div style={{ fontSize: 11, color: "var(--text-secondary)", lineHeight: 1.5, opacity: 0.7 }}>
                    {t("tasks.emptyDesc") || "Use the task button in the input bar to add tasks"}
                  </div>
                </div>
              </div>
            )}

            {/* PRD tasks — grouped by PRD */}
            {!tasksLoading && prdTaskGroups.map(group => {
              const doneCount = group.tasks.filter(t => isDone(t.status)).length
              const totalCount = group.tasks.length
              return (
                <div key={group.prdId} style={{ marginBottom: 12 }}>
                  {/* PRD header */}
                  <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 6, padding: "0 2px" }}>
                    <PriorityBadge p={group.priority} />
                    <div style={{ flex: 1, fontSize: 11, fontWeight: 700, color: "var(--text-primary)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                      {group.prdTitle}
                    </div>
                    <span style={{ fontSize: 10, fontWeight: 600, color: doneCount === totalCount ? "#22c55e" : "var(--text-secondary)", fontFamily: "'JetBrains Mono', monospace", opacity: 0.7 }}>
                  ...
                  {group.tasks.map((task, idx) => {
                    const sc = STATUS_COLORS[task.status] || STATUS_COLORS.pending
                    const isNext = idx === group.tasks.findIndex(t => t.status === "pending") && task.status === "pending"
                    return (
                      <div key={task.id} style={{
                        display: "flex", alignItems: "center", gap: 8, padding: "8px 10px", borderRadius: 8, marginBottom: 4,
                        background: isNext ? (isDark ? "rgba(55,172,192,0.06)" : "rgba(55,172,192,0.04)") : cardBg,
                        border: `1px solid ${isNext ? (isDark ? "rgba(55,172,192,0.2)" : "rgba(55,172,192,0.15)") : isDark ? "rgba(255,255,255,0.05)" : "rgba(0,0,0,0.04)"}`,
                      }}>
                        <div style={{
                          width: 20, height: 20, borderRadius: 5,
                          background: isNext ? "rgba(55,172,192,0.15)" : sc.bg,
                          border: `1px solid ${isNext ? "rgba(55,172,192,0.3)" : sc.border}`,
                          display: "flex", alignItems: "center", justifyContent: "center",
                          fontSize: 8, fontWeight: 700, color: isNext ? "#37ACC0" : sc.text,
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{
                            ...
                            textDecoration: task.status === "done" || task.status === "skipped" ? "line-through" : "none",
                            opacity: task.status === "done" || task.status === "skipped" ? 0.5 : 1,
                            overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap",
                          }}>
                            {task.title}
                          </div>
                          {isNext && <div style={{ fontSize: 7, fontWeight: 700, color: "#37ACC0", marginTop: 1, textTransform: "uppercase", letterSpacing: 1 }}>NEXT</div>}
                        </div>
                        <button onClick={() => {
                          const cycle: Record<string, string> = { pending: "in_progress", in_progress: "done", done: "pending", skipped: "pending" }
                          updateTaskStatus(group.prdId, task.id, cycle[task.status] || "pending")
                        }} style={{
                          padding: "2px 6px", borderRadius: 4, background: sc.bg,
                          border: `1px solid ${sc.border}`, color: sc.text,
                          fontSize: 8, fontWeight: 700, cursor: "pointer", flexShrink: 0, textTransform: "uppercase",
                        }}>
                          {task.status === "in_progress" ? "WIP" : task.status === "skipped" ? "SKIP" : task.status}
                        </button>
                      </div>
            ...
            {/* Standalone tasks (user-created via InputBar) */}
            {!tasksLoading && standaloneTasks.length > 0 && (
              <div style={{ marginBottom: 12 }}>
                {prdTaskGroups.length > 0 && (
                  <div style={{ fontSize: 10, fontWeight: 700, color: "var(--text-secondary)", marginBottom: 6, padding: "0 2px", textTransform: "uppercase", letterSpacing: 1, opacity: 0.6 }}>
                    {t("tasks.manual") || "Manual Tasks"}
                  </div>
                )}
                {standaloneTasks.map((task, idx) => {
                  const sc = STATUS_COLORS[task.status] || STATUS_COLORS.pending
                  const isNext = idx === standaloneTasks.findIndex(t => t.status === "pending") && task.status === "pending"
                  return (
                    <div key={task.id} style={{
                      display: "flex", alignItems: "center", gap: 8, padding: "8px 10px", borderRadius: 8, marginBottom: 4,
                      background: isNext ? (isDark ? "rgba(55,172,192,0.06)" : "rgba(55,172,192,0.04)") : cardBg,
