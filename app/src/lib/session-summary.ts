...
export function isSummaryNoise(text: string | null | undefined): boolean {
...
export function buildSessionDigest(events: AgentEvent[], options: DigestOptions = {}): SessionDecisionDigest {
  ...
  const candidates: Array<{ source: SessionDigestSource; event: AgentEvent | null; summary: string; nextAction: string }> = [
    ...
    },
    {
      source: "user",
      event: firstUser,
      summary: pickMeaningfulText(firstUser?.detail, firstUser?.title, options.fallbackTitle),
      nextAction: "",
    },
  ]
  ...
  const fallbackChosen = {
    ...
    event: null,
    summary: pickMeaningfulText(options.fallbackTitle),
    nextAction: "",
  }
...
function formatDigestLine(digest: SessionDecisionDigest, locale: SummaryLocale): string {
  const label = digest.displayLabel || digest.agentId || (locale === "zh-TW" ? "這個 session" : "this session")
...
export function buildProjectDecisionSummary(digests: SessionDecisionDigest[], locale: SummaryLocale): string {
  ...
  const leadLabel = recommended.displayLabel || recommended.agentId || (locale === "zh-TW" ? "這個 session" : "this session")
  const parts: string[] = []

  if (locale === "zh-TW") {
    if (recommended.status === "blocked") {
      parts.push(`建議先回到 ${leadLabel}：${recommended.summary}`)
    } else if (recommended.status === "working") {
      parts.push(`目前最值得接手的是 ${leadLabel}：${recommended.summary}`)
    } else if (recommended.status === "done") {
      parts.push(`先確認 ${leadLabel}：${recommended.summary}`)
    } else {
      parts.push(`${leadLabel} 最新進度：${recommended.summary}`)
    }
    if (recommended.nextAction) parts.push(`下一步：${recommended.nextAction}`)
  } else {
    if (recommended.status === "blocked") {
      parts.push(`Go back to ${leadLabel} first: ${recommended.summary}`)
    } else if (recommended.status === "working") {
      parts.push(`The best place to continue is ${leadLabel}: ${recommended.summary}`)
    } else if (recommended.status === "done") {
      parts.push(`Check ${leadLabel} next: ${recommended.summary}`)
    } else {
      parts.push(`${leadLabel} latest: ${recommended.summary}`)
    }
    if (recommended.nextAction) parts.push(`Next: ${recommended.nextAction}`)
  }

  const others = digests
    .filter((digest) => digest !== recommended && (digest.summary || digest.nextAction))
...
export function buildProjectSummarySignature(digests: SessionDecisionDigest[]): string {
  return [...digests]
    ...
    .map((digest) => [
      ...
      digest.summary,
      digest.nextAction,
      Math.floor(digest.updatedAt / 1000),
    ].join("::"))
    .join("||")
}
